...
export default {
  ...
  data () {
    return {
      ...
      showHint: false,
      showMatching: false,
      popupName: '自动收款', // 去绑定 去实名 禁止交易 充值匹配成功 提现匹配成功 确认收款 自动收款 被取消 交易完成
      postFormat: {},
      userMsg: {},
      headerInfo: {
        amount_income: 0,
  ...
  methods: {
    ...
    onmessage (info) {
      ...
      // 匹配中 匹配成功 已取消 交易完成 确认收款 未到账 接单用户取消 重新匹配成功 接单用户取消,匹配中 交易完成
      const orderType = info.data.a_status_str
      ...
      if (info.data.order_type === 1) { // 1充值 2提现
        if (orderType === '匹配中' || orderType === '重新匹配成功') {
          stateName = '充值匹配成功'
        }
        this.detailType = '充值'
      } else {
      ...
      // 充值匹配成功 提现匹配成功 确认收款 自动收款 被取消 交易完成
      // stateName = '交易完成'
      setTimeout(() => {
        sessionStorage.setItem('order_amount', info.data.order_amount)
