<template>
  ...
  <div class="main-detail">
      <div class="top-box"
          :class="{
            'blue': (detailInfo.a_status_str == '未到账' && detailInfo.order_type == 1),
        ...
        <template v-if="detailInfo.a_status_str.includes('匹配成功')">
          <count-down class="top-state" endTime="165266200000" endText="xxxx" timeType='zh'></count-down>
        </template>
        <template v-else-if="detailInfo.order_type == 1" >
          <span class="top-state">充值-{{detailInfo.a_status_str | filterRightState}}</span>
        </template>
        <template v-else>
          <span class="top-state">提现-{{detailInfo.a_status_str | takeoutRightState}}</span>
        </template>
      </div>
