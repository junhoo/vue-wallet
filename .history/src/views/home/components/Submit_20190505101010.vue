...
export default {
  ...
  methods: {
    ...
    clickTackout () {
      ...
      console.log(this.homeinfo)
      console.log(this.headerInfo)
      this.keyword = this.homeInfo.amount_income
    },
    ...
    verifyWindow () {
      ...
      if (inputs === '') {
        ...
        if (!exp.test(inputs)) {
          const text = this.selectButton === '充值' ? '充值' : '提现'
          this.showTopHint(`请填写需要${text}的数量`)
        }
        ...
        if (this.selectButton === '充值') {
          ...
          if (this.keyword > parseInt(this.homeInfo.amount_income)) {
            this.keyword = this.homeInfo.amount_income
