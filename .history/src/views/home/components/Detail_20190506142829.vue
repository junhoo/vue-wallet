<template>
  ...
  <div class="main-detail">
    ...
    <div class="top-box"
        :class="{
          'blue': detailInfo.a_status == 1 || detailInfo.a_status == 5,
          'skyblue': detailInfo.a_status == 2 || detailInfo.a_status == 6
        }">
      <span class="top-text">{{detailInfo.a_status | filterLeftState}}</span>
      <span class="top-state">09分27秒</span>
    </div>
    ...
    <div class="content-box">
      ...
      <div class="text-box-4" v-show="type === '充值' || type === '未到账'">
        ...
        <div v-if="detailInfo.choice_pay_type.indexOf('3') >= 0" class="icon-bank"></div>
        <div v-if="detailInfo.choice_pay_type.indexOf('2') >= 0" class="icon-wechat"></div>
        <div v-if="detailInfo.choice_pay_type.indexOf('1') >= 0" class="icon-alipay"></div>
      </div>

      <!-- 备注区 -->
      <div class="note-box" v-show="type === '充值' && detailInfo.a_status.includes('匹配成功')">
        <p>1：请于10分钟内完成付款，超时将自动取消</p>
        <p>2：每天超过3次取消，将被禁止交易48小时</p>
      </div>

      <div class="note-box" v-show="type === '提现' && detailInfo.a_status.includes('匹配成功')">
        <p>1：订单匹配成功，等待买方付款</p>
